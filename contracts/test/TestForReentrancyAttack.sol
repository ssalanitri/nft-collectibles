...
import "../MatrioskaAccessoryFactory.sol";
...
contract TestForReentrancyAttack is IERC1155Receiver {
    ...
    /*function attack(uint256 _totalToMint) external {
        ...
        MatrioskaAccessoryFactory(factoryAddress).mint(1, address(this), 1, "");
        }*/

    // We attempt a reentrancy attack here by recursively calling the
    // MatrioskaAccessoryFactory that created the MatrioskaAccessory ERC1155 token
    // that we are receiving here.
    // We expect this to fail if the MatrioskaAccessoryFactory.mint() function
        ...
        returns(bytes4)
    {
        ...
        if(balance < totalToMint)
        {
            ...
            MatrioskaAccessoryFactory(factoryAddress)
